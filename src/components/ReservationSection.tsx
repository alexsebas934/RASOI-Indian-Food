...
import { Calendar, Users, Clock, Compass, PhoneCall, Check, Sparkle, Star, MessageSquare } from "lucide-react";
...
export default function ReservationSection() {
  ...
  const [successData, setSuccessData] = useState<{ code: string; message: string; bookingText: string } | null>(null);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const msgText = `Hello Rasoi Menifee! I would like to reserve a table:
• Name: ${formData.name}
...
Passcode: ${fallbackCode}`;
    ...
    try {
      ...
      setSuccessData({
        code: fallbackCode,
        message: `Excellent choice, ${formData.name}! Your table space for ${formData.guests} guests is held. Please send your details directly to our reservation number for instant confirmation.`,
        bookingText: msgText,
      });
      ...
      setSuccessData({
        code: fallbackCode,
        message: `Your reservation space details are ready! Send them directly to our reservation phone number to secure your table with Sami.`,
        bookingText: msgText,
      });
    } finally {
      setLoading(false);
          ...
          {!successData ? (
              ...
              <form onSubmit={handleSubmit} className="space-y-5">
                ...
                <div>
                  ...
                  <textarea
                    ...
                    onChange={handleChange}
                    rows={2}
                    placeholder="E.g. Anniversary dinner / gluten sensitivity / celebrating Sami's tandoor!"
            ...
            >
              ...
              <div className="space-y-2">
                ...
                <h4 className="font-heading text-3xl sm:text-4xl text-gold-100 font-semibold mb-2">Almost Confirmed!</h4>
                <p className="text-sm text-gray-300 max-w-md mx-auto leading-relaxed">
                  {successData.message}
                </p>
              </div>

              {/* Instant Text Confirm CTAs */}
              <div className="space-y-3 max-w-sm mx-auto">
                <a
                  href={`sms:+19514492165?body=${encodeURIComponent(successData.bookingText)}`}
                  target="_blank"
                  rel="noreferrer"
                  className="w-full py-3.5 px-4 bg-gradient-to-r from-gold-600 to-gold-400 hover:from-gold-500 hover:to-gold-300 text-luxury-black font-extrabold text-xs uppercase tracking-widest rounded-xl transition duration-300 flex items-center justify-center gap-2 shadow cursor-pointer select-none"
                >
                  <MessageSquare className="w-4 h-4 fill-luxury-black" />
                  <span>Send SMS Text Booking</span>
                </a>

                <a
                  href={`https://wa.me/19514492165?text=${encodeURIComponent(successData.bookingText)}`}
                  target="_blank"
                  rel="noreferrer"
                  className="w-full py-3.5 px-4 bg-emerald-600 hover:bg-emerald-500 border border-emerald-500/30 text-white font-extrabold text-xs uppercase tracking-widest rounded-xl transition duration-300 flex items-center justify-center gap-2 shadow cursor-pointer select-none"
                >
                  <svg className="w-4.5 h-4.5 fill-white" viewBox="0 0 24 24">
                    <path d="M.057 24l1.687-6.163c-1.041-1.804-1.588-3.849-1.587-5.946C.06 5.348 5.397.01 12.008.01c3.202.001 6.212 1.246 8.477 3.514 2.266 2.268 3.507 5.28 3.505 8.484-.004 6.657-5.34 11.997-11.953 11.997-2.005-.001-3.973-.502-5.724-1.455L0 24zm6.59-4.846c1.6.95 3.188 1.449 4.825 1.451 5.436 0 9.859-4.42 9.863-9.864.002-2.637-1.023-5.115-2.883-6.978C16.59 1.901 14.118.874 11.48.874c-5.437 0-9.862 4.42-9.866 9.865-.001 2.028.532 3.82 1.498 5.44l-.986 3.601 3.69-.968c1.55.845 3.125 1.286 4.231 1.286h.001zM17.8 14.8c-.33-.164-1.95-.963-2.25-1.073-.3-.109-.52-.164-.74.164-.22.328-.85 1.073-1.04 1.293-.19.22-.38.246-.71.082-.33-.164-1.39-.512-2.64-1.633-.97-.867-1.63-1.938-1.82-2.265-.19-.328-.02-.505.14-.668.15-.147.33-.382.5-.573.16-.192.22-.328.33-.547.11-.22.05-.41-.03-.574-.08-.164-.74-1.78-.3-1.918-.28-.137-.56-.109-.76-.082-.2.027-.52.109-.79.41-.27.301-1.04 1.011-1.04 2.463 0 1.452 1.05 2.857 1.2 3.048.15.19 2.07 3.161 5.02 4.437.7.304 1.24.485 1.66.619.7.223 1.34.192 1.84.117.56-.084 1.95-.796 2.22-1.528.27-.732.27-1.36.19-1.496-.08-.136-.3-.218-.63-.382z" />
                  </svg>
              ...
              <div className="p-4 bg-luxury-sand max-w-xs mx-auto rounded-2xl border border-gold-500/30 shadow-lg text-center space-y-1">
                <span className="text-[10px] uppercase font-semibold text-gray-400 tracking-wider">Verification Passcode</span>
                <div className="text-xl font-mono text-gold-200 uppercase tracking-widest font-extrabold">{successData.code}</div>
              </div>

              <button
                onClick={() => setSuccessData(null)}
                className="px-6 py-2 text-xs font-semibold uppercase tracking-widest text-gold-400 border border-gold-850/50 hover:border-gold-500 rounded-lg transition"
              >
                Book Another Table
              </button>
            </motion.div>
